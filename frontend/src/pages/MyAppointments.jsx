      ...
      <div>
        {
          appointments.map((item, index) => (
            ...
            <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
              ...
              {!item.cancelled &&  item.payment &&  <button className='sm:min-w-48 py-2 border rounded text-stone-500 bg-indigo-50'></button>}


                {!item.cancelled && !item.payment &&<button onClick={() => appointmentRazorpay(item._id)} className='text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded hover:bg-indigo-400 hover:text-white transition-all duration-300'>Pay Online</button>
                }
                ...
                {item.cancelled && <button className='sm: min-w-48 py-2 border-red-500 text-red-500 rounded'>Appointment cancelled</button>}
