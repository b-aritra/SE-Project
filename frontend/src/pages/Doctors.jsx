...
const Doctors = () => {
  ...
  return (
    <div>
      <p className='text-gray-600'>Choose from the best doctors</p>

      <button className={`py-1 px-3 border rounded text-sm transition-all sm:hidden ${showFilter ? 'bg-indigo-400 text-white' : ''}`} onClick={() => setShowFilter(prev => !prev)}>Filters</button>
      ...
      <div className='w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 gap-y-6 mt-6'>
        {
          filterDoc.map((item, index) => (

            <div key={item._id} onClick={() => navigate(`/appointment/${item._id}`)} className='border border-blue-200 rounded-xl overflow-hidden hover:translate-y-[-10px] transition-all duration-500 cursor-pointer'>
